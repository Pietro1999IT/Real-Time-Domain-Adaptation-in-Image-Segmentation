...
import numpy as np
from utils import poly_lr_scheduler, reverse_one_hot, compute_global_accuracy, fast_hist, per_class_iu, compute_loss, rgb_label
...
def val(args, model, dataloader):
    ...
    with torch.no_grad():
        ...
        for i, (data,label,_,_) in enumerate(dataloader):
            ...
            rgb_image = rgb_label(predict)
            saving_source_rgb = '/content/drive/MyDrive/BiseNetv1-master/dataset/experiment_pred_source' + ".png"   #modified
            rgb_image.save(saving_source_rgb)
            ...
            rgb_image = rgb_label(label)
            saving_file_name_rgb = './dataset/Prova/' + "/labels_rgb/" + "prova" + ".png" 
...
def main(params):
    ...
    dataset_train = cityscapesDataSet(args.dataset, args.data,   crop_size=cropSize,  ignore_label=255, encodeseg = 1)
    dataset_val = cityscapesDataSet(args.dataset, args.val,   crop_size=cropSize, encodeseg=1)
